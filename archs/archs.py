...
def _cnn_submodule(in_channels, out_channels, batch_norm):
    seq = [nn.Conv2d(in_channels, out_channels, 5, 2, 2), nn.ReLU(inplace=True)]
    if batch_norm:
        seq.append(nn.BatchNorm2d(out_channels))
    return nn.Sequential(*seq)

class CNN(SerializableModule):
    def __init__(self, layer_nch, input_size, output_act=ACT_NONE,
                 batch_norm=False):
        ...
        _, x, y = input_size
        for i in xrange(len(layer_nch) - 1):
            self.mseq.add_module('sub%d' % i, _cnn_submodule(layer_nch[i],
                layer_nch[i+1], batch_norm))
            x = (x + 1) // 2
            y = (y + 1) // 2
...
class RegionFC(SerializableModule):
    def __init__(self, input_size, s1_fb_nsize=5, s1_hidden_size=[],
                 s2_azi_nsize=11, s2_hidden_size=[], output_act=ACT_NONE,
                 batch_norm=False):
        super(RegionFC, self).__init__({
            'input_size'     : input_size,
        ...
        osize = next(hsizes)
        s2seq.append(nn.Conv2d(1, osize, kernel_size=(s2_azi_nsize,
                                        nfbank - s1_fb_nsize + 1)))
        isize = osize
        # rest layers 1 by 1 kernel
        for osize in hsizes:
